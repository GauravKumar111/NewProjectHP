({
    ...
    changeInIndexNo : function(component, event,helper){
        console.log("There is a change notice in Index no attribute"); 
        helper.helperRectifySequence(component,event);
    },
    saveEduDetails : function(component, event, helper){
      //call apex function with name
      //SaveEducationDetail
        ...
        var actn =  component.get("c.SaveEducationDetail");
        actn.setParams({ EduDet : EduDetails});
        actn.setCallback(this,function(resp){
            var state = resp.getState();
            if(state == "SUCCESS"){
                console.log( 'Add Educational Details is saved');
               }
            else if(state ==="ERROR"){
